...
contract Wordle {
    event gameCreated(address gameOwner, address secondPlayer, uint256 gameBet);
    event MoveMade(address mover, string answer);
    event gameStarted(address gameOwner, uint256 totalGameBet);
    event gameEnded(
        address gameOwner,
        address secondPlayer,
        uint256 noOfMovesSecondPlayer,
        string outcome
    );

    struct Games {
        bytes32 answer;
        uint256 depositOwner;
        uint256 depositSecondPlayer;
        uint256 noOfMovesOwner;
        uint256 noOfMovesSecondPlayer;
        address secondPlayer;
    ...
    modifier GameShouldNotExist(address owner) {
        require(
            !(games[owner].isValid == true),
            "You have already created a game"
        );
        _;
    }
    ...
    modifier GameShouldExist(address owner) {
        require(
            games[owner].isValid == true,
            "Start This. Game does not exist"
        );
        _;
    }
    ...
    modifier IsPlayerInvolved(address owner) {
        require(
            games[owner].secondPlayer == msg.sender,
            "You are not involved in the game"
        );
        _;
    }
    ...
    function startGame(address secondPlayer, bytes32 secrethash)
        public
        payable
        GameShouldNotExist(msg.sender)
    {
        //make sure the secondplayer is not owner
        require(
            msg.sender != secondPlayer,
            "Please add any other address other than owners"
        );
    ...
    function acceptGame(address gameOwner)
        public
        payable
        GameShouldExist(gameOwner)
        IsPlayerInvolved(gameOwner)
    {
        //check if 24hours have passed since the start of the game
        require(isValidTime(gameOwner));

        //check if the same amount is sent
        require(
            games[gameOwner].depositOwner == msg.value,
            "Please add the sufficient amount"
        );
        ...
        require(
            games[gameOwner].state == State.Created,
        ...
        uint256 totalAmount = games[gameOwner].depositOwner +
            games[gameOwner].depositSecondPlayer;

        emit gameStarted(gameOwner, totalAmount);
    }

    function addMove(address gameOwner, string memory answer)
        public
        GameShouldExist(gameOwner)
    {
        //check if 24hours have passed since the start of the game
        ...
        require(
            gameOwner == msg.sender ||
                games[gameOwner].secondPlayer == msg.sender,
            "You should be the owner or the second player"
        );

        //check if not exceeding 5 moves
        require(
            games[gameOwner].noOfMovesOwner < 6 &&
                games[gameOwner].noOfMovesSecondPlayer < 6,
            "Exceeding the number of moves allowed"
        );
        //add moves
    ...
    function submitAnswer(address gameOwner, string memory secret) public {
        ...
        require(
            ...
            "The game should be in Accepted or Waiting state"
        );

        require(
            keccak256(abi.encodePacked(secret)) == games[gameOwner].answer,
            "Invalid Secret"
        );
        ...
        if (games[gameOwner].state == State.Accepted) {
            ...
            if (
                ...
                //Player 1 won
                payable(gameOwner).transfer(
                    games[gameOwner].depositOwner +
                        games[gameOwner].depositSecondPlayer /
                        2
                );
                payable(games[gameOwner].secondPlayer).transfer(
                    games[gameOwner].depositSecondPlayer / 2
                );
                emit gameEnded(
                    gameOwner,
                    games[gameOwner].secondPlayer,
                    games[gameOwner].noOfMovesOwner,
                    "Player 1 Won"
                );
            } else if (
    ...
    function isValidTime(address gameOwner) public view returns (bool) {
        return block.timestamp - games[gameOwner].timeStarted < 5 * 60;
    }

    function claimFunds(address gameOwner) public GameShouldExist(gameOwner) {
        //check if 24hours have passed since the start of the game
        require(!isValidTime(gameOwner), "24 Hours have not passed");
        ...
        if (msg.sender == gameOwner) {
            require(
                games[gameOwner].depositOwner > 0,
                "You have already claimed the funds"
            );
            payable(gameOwner).transfer(games[gameOwner].depositOwner);
            games[gameOwner].depositOwner = 0;
        } else {
            require(
                games[gameOwner].depositSecondPlayer > 0,
                "You have already claimed the funds"
            );
            payable(msg.sender).transfer(games[gameOwner].depositSecondPlayer);
            games[gameOwner].depositSecondPlayer = 0;
        }
    }
